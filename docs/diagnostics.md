...
| `codex.hooks` | Codex has accepted its hooks. Codex runs none until a human says yes, and an unaccepted hook is skipped in silence — so the wired row, which the older `notify` key alone satisfies, cannot tell you. `warn` until the answer lands in `[hooks.state]`, for **every one** of AgentBar's seven hooks (trust is given hook by hook, and a hook switched off with `enabled = false` does not count). It sees that an answer is recorded, not whether Codex still agrees with it: the hash is Codex's own arithmetic, and a hook Codex considers changed is asked about again at the next session |
