...
| `codex.hooks` | Codex has accepted its hooks. Codex runs none until a human says yes, and an unaccepted hook is skipped in silence — so the wired row, which the older `notify` key alone satisfies, cannot tell you. `warn` until the answer lands in `[hooks.state]`, for **every one** of AgentBar's seven hooks, at the key its own handler occupies: Codex numbers groups and handlers by position in the file, so a user's hooks for the same event come first and their trust does not count. A hook switched off with `enabled = false` does not count either. The warning says whether AgentBar's older `notify` bridge is wired to cover for the hooks meanwhile; when a program of the user's owns `notify`, or AgentBar's line is commented out, it says no Codex session will appear until they are accepted. It sees that an answer is recorded, not whether Codex still agrees with it: the hash is Codex's own arithmetic, and a hook Codex considers changed is asked about again at the next session |
