...
`Scripts/cli/agentbar` is a transliteration of the app with no shared source (one exception: the Codex config reader, `Scripts/hooks/codex/toml.js`, which the CLI loads so that `notify.js` and `doctor` resolve the same trust key), so the
