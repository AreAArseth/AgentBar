...
node --test Scripts/test/*.test.js        # shared homes (state layout 2)
node --test Scripts/remote/test/*.test.js # the remote collector
```
...
| `AGENTBAR_MACHINE_ID_FILE`, `AGENTBAR_BOOT_ID_FILE` | `Scripts/hooks/shared/owner.js` | Read this file instead of the machine's own id — how one disk plays two machines sharing a home. |
| `AGENTBAR_SOURCE_ID=<uuid>` | `owner.js`, the collector | An explicit machine identity (a real knob, for schedulers and cloned nodes, and for tests). |
| `AGENTBAR_REMOTE_HEARTBEAT_MS`, `AGENTBAR_REMOTE_RECONCILE_MS`, `AGENTBAR_REMOTE_STALL_MS` | `Scripts/remote/stream.js` | Shorter clocks, so the stream tests run in seconds. |
...
- the hook directories `doctor` expects (`HOOK_DIRS`, `Diagnostics.hookDirs`), `shared` included
