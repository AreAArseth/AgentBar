...
| `Scripts/test/shell-pid-test.sh` | The pid a row is pruned by, when the host runs the hook through `sh -c`: a node stand-in for the agent runs each hook under dash where it is installed and forks a lone command, or under a stand-in `/bin/sh` that forks the same way (and says so first, since a shell that execs would prove nothing). Every Claude command `install-hooks` writes starts with `exec`, and run the way Claude runs it, each one names the agent. A command line without `exec` still names the agent from the Claude, Gemini, Cursor and Antigravity scripts (Linux, where `/proc` is; skipped elsewhere). Only a `-c` wrapper is stepped over: a script's shell and a direct parent are kept. An older install is repaired in place, one entry per event, the user's own hooks kept, and a user hook that shares a rule with AgentBar's staying in it, and a second run changes nothing | 18 | node; dash when present |
...
./Scripts/test/shell-pid-test.sh
